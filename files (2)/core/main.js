...
/**
 * Encontra um livro pelo ID nos dados locais.
 * Usado pelos botões "Ler" e "Baixar" da interface.
 */
...
/** Renderiza a página inicial (usa apenas dados locais). */
function renderHome() {
  renderApp(HomePage());
}
...
function renderLogin() {
  if (isLoggedIn()) {
    renderApp(ProfilePage(getCurrentUser()));
    return;
  }
  renderApp(LoginPage());
}
...
/**
 * Renderiza a página de uma categoria usando apenas dados manuais.
 */
...
/**
 * Renderiza os resultados de busca a partir do acervo local.
 */
function renderSearchResults(query) {
  const normalizedQuery = query.trim();
  if (!normalizedQuery) return;

  console.log(`[SEARCH] Iniciando busca por: "${normalizedQuery}"`);
  ...
  console.log(`[SEARCH] Total encontrado: ${total}`);
}
...
function initAppDelegation() {
  ...
  app.addEventListener("click", (e) => {
    ...
    if (readBtn) {
      ...
      if (book?.url) window.open(book.url, "_blank", "noopener");
      else
        alert(
          "📚 Este é um livro de exemplo para desenvolvimento.\n\nEm produção, você poderá ler livros reais do acervo público!",
        );
      return;
    }
