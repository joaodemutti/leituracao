      ...
      <div className="container space-y-7">
              ...
              {[
                ...
                <button
                  ...
                  className={`rounded-full px-5 py-2.5 text-sm font-semibold ${
                    scope === value ? "bg-white text-navy" : "bg-white/10 text-white/78"
                  }`}
                >
                  {label}
                </button>
          ...
          {!loading && !error && podium.length > 0 && (
            <div className="mt-10 grid gap-4 md:grid-cols-3">
              {podium.map((entry, index) => (
                <article
                  key={entry.user_id}
                  className={`rounded-[28px] border border-white/10 px-6 py-6 text-center ${
                    index === 0 ? "bg-white/10" : "bg-white/6"
                  }`}
                >
                  <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-gold text-xl font-semibold text-navy">
                    {getInitials(entry.display_name)}
                  </div>
                  <p className="mt-4 text-sm uppercase tracking-[0.16em] text-gold">{entry.rank} lugar</p>
                  <h2 className="mt-2 text-2xl font-semibold text-white">{entry.display_name}</h2>
                  <p className="mt-2 font-serif text-4xl text-gold">{entry.xp_points}</p>
        ...
        {!loading && !error && (
          <section className="panel-card overflow-hidden">
            ...
            {entries.map((entry) => (
              ...
              >
                <div className="text-lg font-serif text-gold">{entry.rank}</div>
                ...
                <div className="text-right font-serif text-3xl text-gold">{entry.xp_points}</div>
              </div>
            ))}
          </section>
        )}
        ...
        {currentUserRow && (
          <section className="panel-card px-6 py-6 text-center">
            <h2 className="font-serif text-3xl text-navy">Suba no ranking</h2>
            ...
            <button
              ...
              className="mt-5 rounded-full bg-navy px-6 py-3 text-sm font-semibold text-white"
            >
